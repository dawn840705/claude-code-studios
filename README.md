...
>
> **v0.6.4:** 결정적 게이트가 **자기가 도는 콘솔을 견디게** 됐습니다. 이 플러그인의 설계 원칙은 「종료 코드가 판정이다」인데, 그 종료 코드가 콘솔 인코딩 하나로 뒤집히고 있었습니다 — 게이트 리포트는 한국어인데 Windows 한국어 콘솔(cp949)이 em-dash 하나를 인코딩하지 못해 `UnicodeEncodeError` 로 죽었고, 그 죽음이 `exit 1` 로 나왔습니다. 규칙대로 종료 코드만 믿은 호출부는 **통과한 작업을 FAIL 로 읽었습니다.** `/humanize` · `/dev-story` · `/remove-bg` 가 각자의 게이트를 부르는 자리가 전부 그랬습니다.
>
