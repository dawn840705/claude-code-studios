...
위치(`_workspace/…/03_metrics`) 기준으로 계산돼 reference/ 로 옮긴 뒤엔
`.claude/.claude/…` 로 깨져 있었고, reference/ 가 이미 sys.path 에 있을 때만
우연히 import 되던 잠재 버그였다.

이 테스트는 reference/ 를 sys.path 에 넣지 않고 metrics_v2.py 를 **파일 경로로
...
_V2 = os.path.join(
    _REPO_ROOT, "skills", "humanize-korean", "reference", "metrics_v2.py"
)


class MetricsV2ImportTests(unittest.TestCase):
    def test_v2_self_resolves_v1_metrics(self) -> None:
        # cwd 를 레포 루트로 둬서 cwd-on-path 로도 reference/ 가 안 잡히게 한다.
