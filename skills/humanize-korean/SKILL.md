...
(개발용 1회성 5종·v2.1 은퇴 5종의 계보와 테스트 시나리오는 `reference/design-notes.md` 참조.)
...
윤문 결과를 사용자 파일에 반영하기 전에 **반드시 먼저 묻는다.** 결과는 기본적으로
응답으로 보여주고, 원문 파일 덮어쓰기는 승인 없이 하지 않는다.

> "윤문 결과를 `<경로>`에 쓸까요? (원문은 `<경로>.orig`로 보존합니다)"
> — May I write the rewritten text to this file?

- 원문 파일을 덮어쓰는 경우 **원문 백업을 함께 만든다.**
- `_workspace/` 아래 중간 산출물(`01_input_with_metrics.txt` 등)은 작업 파일이므로
  승인 없이 써도 된다. 승인이 필요한 것은 **사용자 소유 파일**뿐이다.
