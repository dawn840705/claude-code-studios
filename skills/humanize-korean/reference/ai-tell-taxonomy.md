...
- **적용 범위 (v2.3 명확화)**: 이 규칙은 **영어 원문을 번역·요약하는 맥락** 전용이다. 자생 한국어 산문(영어 원문 없이 쓴 글)에는 발동하지 않는다 — 대조 실측에서 자생 한국어는 오히려 인간이 `그는/그의`를 더 쓰고(AI 0.0 vs 인간 1.9/1000어절), 200자 창 군집(≥3회)도 인간 3.9% vs AI 0.7%로 인간이 더 일으킨다. 요즘 LLM은 자생 한국어 생성 시 대명사를 오히려 억제하므로, 번역 맥락 밖에서 이 규칙을 적용하면 사람 글을 훼손한다. `see: empirical-validation.md#재판단`
...
- _quick: true · quick_pattern: "그/그녀/그것/그들" 단락 3회+ 영어 대명사 직역 · quick_fix: 50% 이상 생략(영형) 또는 호칭-명사구로_
...
- _quick: true · quick_pattern: "A인가, B인가"·"A가 아니라 B" 대구 3회+ 반복 · quick_fix: 한 번만 살리고 나머지는 비대칭 평서문·직접 단언으로_
...
- **블로그·카피·에세이 산문에는 적용하지 않는다 (v2.4 명시)**: 위 장르 가드가 이미 배제하는 범위인데 오독이 반복돼 못 박는다. 그 장르에서 합쇼체(~습니다)와 해요체(~해요)를 문장마다 오가는 것은 **결함이 아니라 규범**이다 — 둘 다 높임이고 격식/비격식으로만 갈린다. 한국어 블로그 포스트 1편 실측 합쇼 5 : 해요 7 = 42:58, 해라체 0문장. E-7을 그 장르로 끌어와 종결을 한쪽으로 통일시키지 말 것 — 장르별 권장 비율은 결함 목록인 이 파일이 아니라 `rewriting-playbook.md` §4 장르표에 있다.
- 처방:
...
- _source_anchor: 김혜영 2019 통번역교육연구 17(2): 133-162 doi:10.23903/kaited.2019.17.2.007 · see_scholarship: scholarship.md#8-종결어미시제서법-처리_
- _quick: true · quick_pattern: 대화-구어 한정 (블로그-카피 산문 제외). 그 안에서 청자 경어법 단계(해라/하게/하오/해요/합쇼) 혼재 · quick_fix: 격식 등급 하나로 일관 유지_
...
- _quick: true · quick_pattern: 권고형 결말 "~해야 한다/~할 필요가 있다" 5회+ · quick_fix: 구체 동사 단언, 주체 명시, 조건문으로 변주_
...
### J-3. 대시(—) 남용 [S1 · 실측 최강 신호] · v2.4 심각도 확정
- 영어 em-dash 스타일 부가 설명. 문장 중간에 줄표를 끼워 넣는 습관.
...
- **실측 (2026-08-05)**: 한국어 위키백과 11편 22.0만 한글자에 산문 삽입 줄표 **0회 — 11편 전부**. 같은 계수로 이 저장소의 클로드 작성 한글 문서 7종 1.59만 자는 **180회**(11.31/1000자). **G²=971**, 종전 최대인 C-8 대구(G²=41.7)의 23배. 인간 관측이 0이라 **배수는 정의되지 않는다.** 한국어 줄표는 맞춤법이 인정하는 부호이지만 실사용이 사실상 없다. 근거·한계·1차 측정 폐기 사유는 `empirical-validation.md` 「J-3 줄표 실측」 참조.
- **세는 것은 산문 삽입뿐**: 마크다운 헤딩 부제·표 셀·인용·코드·목록 표지의 줄표는 **문서 포맷이지 한국어 글쓰기 습관이 아니다.** 1차 계수가 이것을 구분하지 않아 잡힌 것의 74.5%가 구조 문법이었고, 수치를 전면 폐기했다. 목록 항목 *안의* 한국어 문장은 산문으로 센다.
- **결정적 게이트**: `verify_gates.py` **P5**. 절대치가 아니라 before/after 로 판정한다. (a) 원문 3회 이상인데 윤문본이 2회 초과면 미달, (b) 윤문본이 원문보다 늘었으면 **개수와 무관하게** 역방향 삽입. 둘 다 exit 1(WARN, 채택 금지가 아니라 고지+finalize 승급). **판정은 exit code 가 하고, 이 항목의 서술은 그것을 설명할 뿐이다.**
  - 이 설계가 "원문 보존" 예외를 완전히 면제하지는 않는다. 원문에 **산문 삽입** 줄표가 3회 이상이면 그대로 보존해도 (a)에 걸린다. 실측상 그런 한국어 원문은 22.0만 자에 0건이라 드물고, 걸려도 WARN 이므로 사람이 보고 넘기면 된다. 대신 원문 1~2회를 보존하는 흔한 경우는 스킵되고, 없던 줄표를 넣는 것은 1회부터 잡힌다.
- _quick: true · quick_pattern: 한국어 산문 문단 안에 끼운 줄표(—) 3회 이상 (헤딩, 표, 코드, 목록 표지는 셈에서 빠짐) · quick_fix: 쉼표, 괄호, 별도 문장으로 분해. 원문에 이미 있던 대시는 보존_
...
### J-5. 가운뎃점(·) 나열 남용 [S3 · estimated] · v2.4 신규
- 패턴: 블로그·카피에서 한 문단 2회+ 반복될 때만. 개별 사용은 정상
- **개별 사용은 판정 대상이 아니다.** 가운뎃점은 한글 맞춤법 문장부호 규정이 인정하는 부호이고, 규정이 드는 세 용법 — **열거**(사과·배·복숭아), **짝 결합**(동사·형용사, 공주·논산), **공통 성분 생략**(우리말·글, 상·하반기) — 은 항목 길이를 제한하지 않는다. `대법원장·대법관`, `국가·지방자치단체`, `민족적·역사적` 은 전부 정상 표기다. **한 줄만 보고 AI 티라고 판정하지 말 것.** 신호는 개별 나열의 정당성이 아니라 문서 전체의 밀도다.
- **장르 가드 (C-1·E-6·E-7 선례)**: 블로그·마케팅 카피·에세이 등 **독자용 산문에만 적용한다.** 백과·법령·보도·학술·연감 레지스터는 가운뎃점을 관례적으로 많이 쓰며 그 자체로 정상이다 — 이 장르에서는 발동하지 않는다.
- **발동 조건은 세는 것 하나뿐이다**: 위 장르에서 **한 문단에 가운뎃점이 2회 이상** 나타날 때. 각 나열이 규정 용법에 해당하는지는 **묻지 않는다** — 그 물음은 답이 정해지지 않아(모든 나열은 어떤 의미로든 열거다) 판정을 모델 재량으로 되돌린다. 세지 못하면 발동하지 않는다.
- 예: (AI) "탐지·윤문·자체검증을 일괄 처리한다" → (사람) "탐지와 윤문, 자체검증을 한 번에 처리한다"
- 처방: 그 문단에서 **하나만** 쉼표나 "와/과"로 푼다. 나머지는 그대로 둔다. **전량 치환 금지** — 자연스러운 한국어 나열까지 해체한 C-1 사고(v2.0.1)와 같은 실패 모드다. 가운뎃점을 "와/과"로 바꾸는 것 자체는 한국어를 깨뜨리지 않으므로, 밀도만 낮추면 목적은 달성된다.
- **절대 손대지 않는 것 (표층으로 판별 가능)**: (a) 항목 중 한 글자짜리가 있는 축약 — 한·미, 노·사·정, 우리말·글, 상·하반기. (b) 숫자가 결합한 고유명사 — 3·1 운동, 6·25, 4·19. (c) 원문에 이미 있던 가운뎃점. J-3(대시)의 원문 보존 예외와 같은 취지.
- C-1(열거 표지)과 축이 다르다 — C-1은 "첫째·둘째" 같은 **표지어**, J-5는 **문장부호**다. 한 나열이 둘 다 위반할 수 있으므로 중복 계상하지 않는다.
- **metric 없음 (설계 실패를 기록으로 남긴다)**: v2.4에서 `interpunct_list_rate`(항목 길이 임계)를 만들었다가 **폐기했다.** 한국어 위키 8편(18만 자)에서 사람이 쓴 가운뎃점 런 268건 중 160건(59.7%)이 양성이었고, 국립국어원 용례 10건 중 9건이 오탐이었다. 길이로는 갈리지 않는다 — 3음절 한자어가 한국어의 최빈 명사 형태라서 임계가 정상 표기를 그대로 삼킨다. 상세 근거와 재시도 금지 사유는 `empirical-validation.md` 「J-5 길이 임계 기각」 참조. **이 패턴은 문서 레벨 판단이지 스크립트 판정이 아니다.**
- **estimated 플래그 (E-7 선례)**: 인간 대조 측정에서 확인된 것은 "길이 임계가 틀렸다"까지이고, 밀도 자체의 판별력(AI/인간 배수, G²)은 아직 미상이다. **S3 고정, S2 이상 승격 금지** — 승격은 `empirical-validation.md` 재측정 회차 이후에만.
- _quick: false_
...
`reference/metrics_v2.py` 14개 신규 함수가 3축을 운영한다(모든 metric에 `speculative: true` 플래그 권고):
...
- **v2.4** (2026-08-05): **본진 신규 1건 — `J-5` 가운뎃점(·) 나열 남용 [S3 · estimated], `quick: false`**. J 카테고리 유일한 미커버 부호였다.
  - **metric을 만들었다가 측정으로 기각했다.** 초안 `interpunct_list_rate`는 "한국어는 가운뎃점 항목을 1~2음절로 줄여 쓴다"를 전제로 항목 길이를 임계로 삼았다. 그 전제가 틀렸다 — 축약을 요구하는 것은 규정의 **공통 성분 생략** 용법뿐이고, **열거**·**짝 결합**은 항목 길이를 제한하지 않는다. 한국어 위키 8편(18만 자)에서 사람이 쓴 런 268건 중 160건(59.7%) 양성, 국립국어원 용례 10건 중 9건 오탐. metric·baseline 셀·테스트 전량 철회했고, 근거는 `empirical-validation.md` 「J-5 길이 임계 기각」에 남겨 재시도를 막는다.
  - **남은 것은 문서 레벨 산문 규칙이다.** J-3(대시)와 같은 형태 — 개별 사용이 아니라 문서 전반의 밀도를 보고, 블로그·카피·에세이 장르에만 발동하는 가드를 달았다(C-1·E-6·E-7 선례). 백과·법령·보도·학술은 가운뎃점을 관례적으로 쓰므로 제외.
  - **심각도는 S3**, `estimated`. 측정이 확인한 것은 "길이 임계가 틀렸다"까지이고 밀도 자체의 판별력은 미상이다. 재측정 전까지 S2 이상 승격 금지.
  - **`J-3` 심각도 S1 확정 + `verify_gates.py` P5 결정적 게이트 신설** — 산문 삽입 줄표(em dash) 실측: 한국어 위키 11편 22.0만 자에 **0회** vs 이 저장소의 클로드 작성 한글 7종 1.59만 자에 **180회**(11.31/1000자). **G²=971**, 종전 최대 C-8(41.7)의 23배. J-5(가운뎃점)와 정반대 결과다 — 같은 J 카테고리 부호인데 하나는 사람이 많이 쓰고 하나는 거의 안 쓴다. 실측 없이는 알 수 없었다.
  - **1차 측정은 폐기했다.** 첫 계수(619배·G²=2290)는 잡힌 것의 74.5%가 마크다운 구조 문법(불릿·헤딩·표·인용)이었고, 인간 코퍼스는 렌더 평문이라 그 형태를 만들 수 없다. 한국어 습관이 아니라 문서 포맷을 재고 있었다. `em_dash_count` 를 산문 전용으로 다시 구현하고 두 코퍼스를 재측정했다 — 결론(S1·게이트)은 유지되지만 **수치는 전부 교체됐다.** 새 지표를 만들 때 "잡힌 것을 표본으로 뜯어 무엇이 잡혔는지 분류하는" 단계를 건너뛴 결과다.
  - **`E-7` 장르 경계 명시 (축은 그대로)** — 세 번째 프롬프트("해요체와 습니다체 5:5")가 E-7과 충돌하는 것처럼 보였으나, E-7의 장르 가드는 처음부터 **대화·구어 한정**이고 블로그 산문은 그 밖이다. 축을 높임/낮춤 이진으로 접는 초안을 냈다가 되돌렸다 — 그렇게 하면 하오·하게체가 축에서 사라지고, 소설 대화·인터뷰에서 같은 화자의 합쇼↔해요 교체(캐릭터 말투 붕괴)를 놓친다. **패턴 ID·심각도·검출 임계·`estimated` 모두 불변.** 블로그의 합쇼↔해요 교체가 규범이라는 것은 결함 목록이 아니라 `rewriting-playbook.md` §4 장르표에 적었다.
...
- **v1.3** (2026-04-25): **본진 신규 1건 (C-9 숫자 괄호 인덱싱) + 본진 보강 1건 (I-2 결합형 변종)**, 그리고 **서브 패턴 발굴 운영 체계 도입**. v1.2 이후 멈춰 있던 패턴 발굴이 새 인프라로 깨진 회차:
  ...
  - **운영 인프라 5종 신설**:
  - **candidate 풀 신설** (`reference/pattern-candidates.md`) — detector·rewriter·naturalness-reviewer가 미분류 의심 패턴을 단일 그릇에 누적. 임시 ID(`cand-{대분류}-{YYYY}-{NNN}`)·4상태(pending/promoted/rejected/merged)·기각 사유 5종 라벨·90일 미재현 자동 만료 정책
  ...
  - **외부 샘플 수집 파이프라인** (`reference/sample-collection.md`) — 4축 다양성 매트릭스(모델·장르·길이·작가), 4종 채널(사용자 자발·합성 샘플·공개 데이터·외부 contributor), 익명화·저작권 5대 정책
  - **승격 자동 검증 체크리스트** (`reference/promotion-checklist.md`) — 6개 게이트(사전 점검·재현·본진 중복·분류 적합성·처방 적합성·본진 위계). 일부 게이트(0.2·0.3·1.1·1.2·5.2)는 향후 스크립트 자동화 가능
  - **v1.3 발행 전 파일럿 회차 결과**:
...
- **v1.2** (2026-04-25): Issue #1(simonsez9510) 후속 — 패턴 신설 0건, **권한 위계와 운영 체계 추가**:
  ...
  - **`author-context.yaml` 스키마** 신설 (`reference/author-context-schema.md`) — opt-in voice profile 주입 양식, Schema validator 책임(무력화 불가 disable 거부, multiplier 캡 위반 거부, prompt injection escape character 검증), Telemetry 정책(`voice_profile_log.json`)
  ...
  - **다운스트림 caller reference** — `reference/proposals/`(PR #3, simonsez9510 어댑터 reference, 메인테이너 SSOT 외부 격리)
