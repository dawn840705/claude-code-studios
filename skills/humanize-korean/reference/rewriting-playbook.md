...
> Toral 2019·Baker 1993·Toury 1995 + 한국 PE 가이드라인(윤미선 외 2018·김혜림 2022·이상빈 2017·2018a·2018b·마승혜 2018) 통합. 본진 패턴 ID에 처방을 묶어 윤문가가 한 번에 적용 가능한 형태로 압축. 학술 출처 전문은 `reference/scholarship.md`.
...
| 블로그 포스트 | 친근한 어조, 질문형, **합쇼체(~습니다)와 해요체(~해요)를 문장마다 오가기** | 기계적 "첫째/둘째" 공식, 종결을 한쪽으로 통일 |
| 공적 연설·축사 | 격식체, 문어체 | 구어체·이모지·불릿 |
| 백과·법령·보도·학술 | 가운뎃점 나열, 절 번호, 긴 관형구 | 구어체, 이모지 |

윤문가는 입력 첫 100자를 읽고 장르를 추정한 뒤 이 표로 허용/금기 선을 조정한다.

마지막 행은 **J-5(가운뎃점)의 장르 가드**를 표로 옮긴 것이다. 이 레지스터는
가운뎃점을 관례적으로 쓰므로 밀도가 높아도 정상이다 — 발동시키지 않는다.

블로그 행의 종결 혼용은 **E-7의 반대편**이다. E-7(청자 경어법 일관성)은 장르
가드상 대화·구어 전용이고, 블로그 산문은 그 밖이다. 이쪽에서는 합쇼체와 해요체를
오가는 것이 규범이고 **한쪽으로 통일된 종결이 오히려 AI 티**다(실측 블로그 1편
합쇼 5 : 해요 7). 대략 반반이면 자연스럽다. 다만 소설 대화·인터뷰 트랜스크립트는
E-7의 적용 범위이므로 여기 규칙을 끌어다 쓰지 말 것 — 같은 화자가 같은 청자에게
합쇼와 해요를 오가면 그쪽에서는 말투 붕괴다.
