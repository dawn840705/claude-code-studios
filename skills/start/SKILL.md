...
Check:
- **Engine configured?** Read `.codex/studio/technical-preferences.md`. If the Engine field contains `[TO BE CONFIGURED]`, the engine is not set.
...
This is the first thing the user sees. Ask the user directly with these exact options so the user can click rather than type:
...
1. Ask them to describe their concept in one sentence — genre and core mechanic. Use plain text, not direct user question (it's an open response).
2. Acknowledge the concept, then ask the user directly to offer two paths:
...
After presenting the recommended path, ask the user directly to ask the user which step they'd like to take first. Never auto-run the next skill.
