...
2026-09-19 에 한 세션이 이런 상태였다 — 캐시에 `0.6.5` 가 온전히 있고
(에이전트 45 · 스킬 90), 프로젝트 설정에 `enabledPlugins` 가 `true` 이고,
`verify_trajectory.py` 가 `exit 0` 을 냈다. 그런데 **훅도 에셋도 스킬도 하나도
안 올라왔다.** 호스트의 `installed_plugins.json` 에서 등재 항목이 유실돼,
«이걸 켜라» 는 설정이 아무것도 안 가리키고 있었다.

아무도 에러를 내지 않은 이유는 단순하다. **기존 게이트는 전부 디스크의 사본을
본다.** 그 사본이 실제로 호스트에 **연결돼 있는가**는 아무도 묻지 않았다.
v0.6.4 회고와 같은 병이 한 층 위에서 재발한 것이다 — 그때는 검사 층위가
`import` 라 CLI 실패를 못 봤고, 이번엔 검사 층위가 디스크라 로드 실패를 못 봤다.

- **`scripts/verify_install.py`** — 호스트 레지스트리를 읽어 한 가지만 답한다:
  이 플러그인이 등재돼 있고, 그 등재가 **실재하는 경로**를 가리키는가.
  `0` 정상 · `1` 낡음(등재 version ≠ 가리키는 `plugin.json` 의 version) ·
  `2` 로드 불가(등재 없음, 또는 모든 경로가 사라짐 = 9-19 상태) ·
  `3` 판정 불가(레지스트리를 못 읽음). `CLAUDE_CONFIG_DIR` 을 존중한다 —
  설정 위치를 옮겼다고 «미등재» 로 읽으면 게이트가 통째로 꺼진다.
- **`3` 의 범위를 좁게 잡았다.** «레지스트리를 못 읽었다» 만 `3` 이고,
  «거기에 이 플러그인이 없다» 는 `2` 다. 둘을 합치면 이 게이트가 잡으려던
  바로 그 버그가 다시 숨는다.
- **훅이 될 수 없고, `test.yml` 에도 넣지 않았다.** 이 사고가 나면 **훅이
  안 도는 것**이 증상이라 훅 기반 검사는 정작 필요한 순간에 침묵한다. CI
  러너에는 호스트 레지스트리가 없어 정직하게 `3` 이 나오므로, 항상
  판정불가인 스텝은 커버리지가 아니라 소음이다. CI 에서의 자리는
...
`CLAUDE.md § 작업 원칙`에 새 규칙을 추가했다. 이제 작업을 시작하기 전에 (1) 이번
세션에 Haiku/Sonnet/Opus 중 무엇이 효율적인지, (2) 에이전트를 스폰한다면 Agent
도구 `model` 파라미터에 무엇을 쓸지 한 줄로 추천한다. 추천은 제안일 뿐이다 —
세션 모델 전환은 사용자가 직접 하고, 에이전트 모델 추천은 기존
[Route hint](CLAUDE.md#route-hint--pick-a-route-before-spawning)의 "비용 절감은
호출 수를 줄이는 데서 온다, 모델을 몰래 낮추지 않는다" 원칙을 따른다 — 이 규칙이
그 원칙을 무력화하지 않도록 명시적으로 연결해뒀다.
...
Codex 단독, 둘이 분업. 셋 다 "풀 스튜디오로 행동한다"가 정답이 아닌데 세션이 그걸
알아낼 방법이 없었다. `rules/runtime-modes.md` 가 그 자리를 만든다.

- **모드는 `production/runtime.txt` 한 줄이 정본이다** (`claude` · `codex` ·
  `split`, 없으면 `claude`). 프로젝트에 `AGENTS.md` 나 `.codex/` 가 보인다는 건
  Codex 가 언젠가 거기 설정됐다는 뜻이지 지금 이 작업을 하고 있다는 뜻이 아니다 —
  `track.txt` 가 존재하는 이유와 같은 실패다. 두 축은 독립: `track.txt` 는 무엇을
  만드는지, `runtime.txt` 는 누가 만드는지.
- **분업 모드는 덮어쓰기 위험이다.** `subagent-collaboration.md` § 2.1 의 잃어버린
  편집 — 성공했다고 보고되는 유일한 실패 — 이 한 층 위에서 더 나쁘게 재현된다. 두
  런타임은 세션도 컨텍스트도 도구 로그도 공유하지 않으므로, 작업 전 **경로 단위**
  소유권 분할을 요구한다 (주제 단위 금지 — 주제는 파일 쓰기와 대조할 수 없다).
- **강점 분담은 비워뒀다.** 분업의 방법은 규칙이지만 내용은 미확정이고, 규칙이 그걸
  지어내지 않는다고 명시했다.
- `CLAUDE.md` 에서 이 절은 도메인 팩 판단보다 **앞**에 둔다 — 어떤 에이전트를 띄울지
  이전에 행동할지 말지가 먼저다.
