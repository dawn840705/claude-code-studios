...
2. **Present 2-4 options with reasoning:**
   ...
   - *After the full explanation, ask the user directly to choose*
...
Ask the user directly after presenting options instead of relying on
...
- When running as a Codex subagent (tool may not be available) — structure your
  text output so the orchestrator can present options with a concise direct question
