...
91 skills organized by phase. Invoke one explicitly as `$skill-name` in Codex.
...
| Command | Coordinates |
|---------|-------------|
| `$studio-orchestrator` | Selects role guides, delegates bounded Codex subagent work, integrates evidence, and applies quality gates |
...
| `$humanize` | Thin entry skill for the above — Fast mode by default, `--strict` forces the 3-call precision path. Invoke explicitly in Codex. |
