...
| Rule File | Enforces |
| ---- | ---- |
| `runtime-modes.md` | Which runtime is working this project, and therefore whether this plugin runs the **full studio** (`claude`), **stands down** (`codex`), or works a **partition** (`split`). Read from `production/runtime.txt`, never inferred from an `AGENTS.md`/`.codex/` sighting — the same reason `track.txt` exists: the ambient signal is missing exactly when the question first matters. In `split` mode, write ownership is partitioned **by path** before work starts — `subagent-collaboration.md` § 2.1's lost-Edit failure, one level up and worse, because the two runtimes share no session, context or tool log. Which strengths go where is **undecided** and the rule refuses to invent it |
