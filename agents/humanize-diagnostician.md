...
- `taxonomy_path`: `.../reference/diagnosis-rules.md` — 진단 전용 슬림 인덱스(71패턴 전수: ID·정의·탐지 시그니처). SSOT `ai-tell-taxonomy.md`에서 자동 생성되며, 진단에 불필요한 예문 전수·처방·버전주석을 뺀 것이다. 전량 taxonomy 로드는 진단 계약(정확한 ID + 지배도)에 불필요.
