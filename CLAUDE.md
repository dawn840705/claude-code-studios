...
- **작업에 들어가기 전에, 이번 작업에 어떤 모델이 효율적인지 한 줄로 추천하세요.** 대상은 두 가지입니다: (1) 지금 세션 자체에 쓸 모델(Haiku/Sonnet/Opus) — 단순 반복 작업엔 가벼운 모델, 설계 판단이 필요한 작업엔 무거운 모델. (2) 에이전트를 스폰한다면 Agent 도구의 `model` 파라미터로 지정할 서브에이전트 모델. 둘 다 **제안일 뿐 자동 전환이 아닙니다** — 세션 모델은 사용자가 직접 바꾸고, 에이전트 모델 추천은 [Route hint](#route-hint--pick-a-route-before-spawning)의 "비용 절감은 호출 수를 줄이는 데서 온다, 모델을 몰래 낮추지 않는다" 원칙을 따릅니다.
